...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Form Validation and Submission (moved inside)
    if (contactForm) {
    ...
    // Email validation helper (moved inside)
    function isValidEmail(email) {
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        return emailRegex.test(email);
    }

    // Intersection Observer for scroll animations (moved inside)
    ...
    // Observe elements for animation (moved inside)
    document.querySelectorAll('.about, .projects, .contact').forEach(section => {
        observer.observe(section);
    });

    // Add animation classes to CSS (moved inside)
    ...
    // Circular Scroll Indicator with Scroll to Top (moved inside)
    ...
    // Scroll to top on click (moved inside)
